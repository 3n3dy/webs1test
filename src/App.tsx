...
const CANVAS_ICONS = [FileText, Folder, Laptop, Brain, Smartphone, PenTool, Coffee, Book, Calendar, Lightbulb, Paperclip];
  ...
  useEffect(() => {
    ...
    const createIconPath = (icon: any, size: number) => {
      ...
      if (icon === FileText) {
        ...
        path.roundRect(-size * 0.25, -size * 0.5, size * 0.5, size, size * 0.08);
  ...
  return (
    <div className="-mt-8 relative min-h-screen bg-gradient-to-br from-slate-900 via-purple-800 to-pink-800 overflow-hidden">
        ...
        <div className="text-center max-w-5xl mx-auto space-y-8">
          ...
          >
            ...
            <Suspense fallback={<div className="h-[72px] w-[200px]" />}>
              <ProjectInfoModal isOpen={isProjectInfoOpen} onOpenChange={setIsProjectInfoOpen} />
            </Suspense>
          </motion.div>
          <motion.div
            initial={{ opacity: 0 }}
      ...
      <div className="absolute bottom-10 left-1/2 transform -translate-x-1/2 animate-bounce">
        <svg width="50" height="85" viewBox="0 0 50 85" className="drop-shadow-md">
          {/* Два кружечки */}
          ...
          <path
            d="M 8,18 L 42,18 L 28,42 L 22,42 Z"
            fill="transparent"
          />

          {/* Нижня частина - БІЛА */}
          <path
            d="M 22,42 L 28,42 L 42,72 L 8,72 Z"
            fill="white"
          />
          ...
          <polygon
            points="11,21 39,21 25,40"
            fill="none"
          />

          {/* Центральна лінія */}
          <line
            x1="25" y1="40" x2="25" y2="44"
            stroke="#1f2937"
            strokeWidth="2"
          />
...
const PainSolutionSection = memo(() => {
  ...
  const pains = useMemo(() => [
    ...
    },
    {
      ...
      solution: "Формування загальної бази знань та під кожну посаду, автоматичне оновлення та версіонування"
    },
    {
      icon: "❓",
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-3 gap-8 items-start">
          {pains.map((pain, index) => (
            ...
            >
              <div className={`text-6xl mb-4 transition-all duration-300 ${activePain === index ? 'scale-110' : ''}`}>
                {pain.icon}
              </div>
              <h3 className="text-2xl font-semibold text-gray-900 mb-4">{pain.title}</h3>
...
const PackagesSection = memo(() => {
  ...
  const allColumns = useMemo(() => [
    // Колонка 1 - для Картки 1, Ліва позиція
    {
      items: [
        {
          label: (<>Адаптація / Результат</>),
          value: (<>Цифровий порядок: Створення логічної та інтуїтивно зрозумілої архітектури папок (структура, де кожен файл має своє місце). Швидкий старт: Готовий чек-лист для адаптації нового співробітника, що дозволяє власнику не пояснювати базові речі «на пальцях». Розробка до 5 ключових інструкцій або регламентів (наприклад, «Як ми зберігаємо файли», «Правила комунікації»).</>)
        },
        {
        ...
        },
        {
          ...
          value: (<>5 годин підтримки та консультацій після впровадження системи для корекції процесів та відповідей на запитання команди.</>)
        }
      ]
    },
      ...
      items: [
        {
          ...
          value: (<>Глибина: Поверхнева систематизація існуючих активів (документів). Ми не змінюємо вашу стратегію, ми наводимо лад у тому, що вже є. Фокус: Архітектура хмарного сховища та критично важливі документів. Створення базової структури бази даних (папки, теги, рівні доступу). Обсяг: Опрацювання до 20 ключових файлів/документів компанії. </>)
        },
        {
          label: (<>Інновації / Інструменти</>),
          value: (<>Використання перевіреного безкоштовного софту під специфіку бізнесу, що не створює додаткового фінансового навантаження на бюджет: Google Workspace (систематизація Drive). Інтеграція з Notion, xTiles або OneNote (базове структурування даних). Впровадження до 3 нових інструментів на вибір (таск-менеджери, календарі тощо).</>)
        }
      ]
    },
    // Колонка 3 - для Картки 1, Права позиція
    {
      items: [
        {
          label: (<>Фішка пакету:</>),
          value: (<>Чек-листи адаптації. Це не просто список справ, а перший крок до вашої майбутньої корпоративної «Вікіпедії». Він знімає з власника головний біль при наймі перших асистентів або дає чіткий план дій для помічника.</>)
        },
        {
          label: (<>Орієнтовна вартість:</>),
          value: (<>$500 (21,000 – 25,000 грн). Фінальна пропозиція щодо бюджету буде підготовлена після завершення аудиту та погодження плану дій.</>)
        },
        {
          label: (<>Ідеально підходить</>),
          value: (<>Для мікробізнесу, малих команд, та соло підприємців які відчувають «інформаційну задуху»: документи розкидані по поштах та месенджерах, а кожен новий файл створює ще більше хаосу в Google Drive.</>)
        }
      ]
    },
    // Колонка 4 - для Картки 2, Ліва позиція
    {
      items: [
        {
          label: (<>Ідеально підходить:</>),
          value: (<>Для вже зростаючих компаній та команд 7-12 чоловік. Коли власник вже не може контролювати кожного особисто і потребує, щоб система сама «навчала» та «спрямовувала» працівників.</>)
        },
        {
          label: (<>Супровідні години</>),
          value: (<>8 годин підтримки після впровадження. Цього достатньо для «тонкого налаштування» системи під реальні відгуки команди.</>)
        },
        {
          label: (<>Інновації / Інструменти</>),
          value: (<>Smart-система: Впровадження Таск-менеджера на вибір як основного робочого простору. ШІ-помічники: Базова інтеграція Gemini або ChatGPT у робочі процеси для генерації контенту, відповідей клієнтам або аналізу текстів. Впровадження до 5 нових інструментів для автоматизації рутини.</>)
        }
      ]
    },
    // Колонка 5 - для Картки 2, Середня позиція
    {
      items: [
        {
          label: (<>Адаптація / Результат</>),
          value: (<>Корпоративна Вікіпедія: Створення повноцінної бази знань, де зібрані всі правила та стандарти компанії. Автоматизована адаптація: Розробка персональних баз знань під конкретні посади (Продажі, Склад, Адмін) та система тестів для перевірки знань стажерів. Розробка до 5 ключових інструкцій або регламентів (наприклад, «Як ми зберігаємо файли», «Правила комунікації», «Інструкція посади»).</>)
        },
        {
          label: (<>Масштабування</>),
          value: (<>Система розроблена з можливістю масштабування до 100+ користувачів без зміни архітектури.</>)
        },
        {
          label: (<>Орієнтовна вартість</>),
          value: (<>$1,200  (51,800 – 65,000 грн). Фінальна пропозиція щодо бюджету буде підготовлена після завершення аудиту та погодження плану дій.</>)
        }
      ]
    },
    // Колонка 6 - для Картки 2, Права позиція
    {
      items: [
        ...
        {
          label: (<>Фішка пакету</>),
          value: (<>Діалог зі «Звичаями»: Через особисті інтерв'ю з кожною посадою я виявляю приховані проблеми, які не видно в звітах. Це дозволяє впровадити ШІ саме туди, де він звільнить найбільше часу.</>)
        },
        {
          label: (<>Обсяг роботи / Глибина занурення</>),
          value: (<>Глибина: Середня. Ми занурюємося в людський фактор через інтерв'ю та досліджуємо наявну базу знань, процеси CRM чи інших програм що використовуються. Фокус: Створення «командного розуму» та усунення дублювання функцій. Обсяг: Обробка до 35 ключових документів та аналіз існуючої ERP/CRM-системи.</>)
        }
      ]
    },
    // Колонка 7 - для Картки 3, Ліва позиція
    {
      items: [
        {
          label: (<>Інновації / Інструменти </>),
          value: (<>Універсальний стек (All-in-one). Працюємо з будь-якими інструментами, що потрібні саме вам: CRM + Task Managers (Asana, ClickUp, Notion тощо). Глибока AI-інтеграція: ШІ не просто як чат, а як незамінний помічник для команди (звіти, аналіз запитів, генерація довідок).</>)
        },
        {
          label: (<>Адаптація / Результат </>),
          value: (<>Компанія стає «прозорою» та керованою. Автоматизований onboarding: Нові люди навчаються самі через базу знань. Підготовка асистента: навчаємо вашого співробітника бути «охоронем системи», щоб вона не розвалилася після мого виходу. Корпоративна Вікіпедія: Створення повноцінної бази знань, де зібрані всі правила та стандарти компанії. </>)
        },
        {
          label: (<>Термін впровадження</>),
          value: (<>2–3 місяці (залежно від складності «воскресіння» або масштабування процесів). </>)
        }
      ]
    },
    // Колонка 8 - для Картки 3, Середня позиція
    {
      items: [
        {
          label: (<>Ідеально підходить</>),
          value: (<>Для будь-якого масштабу та стадії бізнесу. Стартапи: Побудова фундаменту з нуля, щоб не «гасити пожежі» пізніше. Трансформація: Коли компанія змінює стратегію або напрямок. Реанімація: «Воскресіння» процесів, які перестали працювати або застаріли. Вихід з операційки: Для власників, що прагнуть передати управління системі.</>)
        },
        {
          label: (<>Фішка пакету</>),
          value: (<>Універсальність «любе-голубе»: Цей пакет не обмежений кількістю людей чи галуззю. Це повна архітектурна розробка: ми беремо те, що є (навіть якщо це хаос), і перетворюємо на впорядковану систему, що працює, або створюємо її.</>)
        },
        {
          label: (<>Супровідні години </>),
          value: (<>15 годин підтримки для шліфування процесів, навчання керівної ланки та фіксації «звичаїв» компанії. </>)
        }
      ]
    },
    // Колонка 9 - для Картки 3, Права позиція
    {
      items: [
        {
          label: (<>Обсяг роботи / Глибина занурення</>),
          value: (<>Глибина: Максимальна. Повний аналіз бізнес-процесів, культури та операційної моделі компанії. Фокус: Побудова або повна реконструкція системи управління знаннями та процесами. Обсяг: Необмежена кількість документів, повна інтеграція всіх систем компанії.</>)
        },
        {
          label: (<>ROI</>),
          value: (<>Середня окупність за 6-8 місяців за рахунок підвищення продуктивності та скорочення втрат часу.</>)
        },
        {
          label: (<>Орієнтовна вартість </>),
          value: (<>Від $2,500 (107,500 грн) Фінальна пропозиція щодо бюджету буде підготовлена після завершення аудиту та погодження плану дій. </>)
        }
      ]
    }
  ], []);
  ...
  return (
    <div id="packages" className="py-24 bg-gradient-to-b from-white to-purple-50">
      <div className="max-w-7xl mx-auto px-6">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
        ...
        <div className={`${activePackage !== null ? 'rounded-3xl overflow-hidden shadow-2xl' : ''} relative`}>
          {activePackage !== null && (
            <div className={`absolute inset-0 bg-gradient-to-r ${activePackage === 0
          ...
          {activePackage !== null && (
            <div className="relative z-10 pt-10 pb-8 px-8">
              <div className="flex items-center justify-center gap-4 mb-4">
                ...
                <span className="text-white/60 text-sm uppercase tracking-wider">Пакет послуг</span>
                <div className="w-16 h-0.5 bg-white/30"></div>
              </div>
          ...
          <div className={`grid grid-cols-1 md:grid-cols-3 transition-all duration-500 ${activePackage !== null ? 'gap-0' : 'gap-8'
            ...
            {packages.map((pkg, pkgIndex) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {/* FRONT */}
                    <div className={`flip-card-front w-full bg-white rounded-3xl shadow-xl cursor-pointer flex flex-col ${pkg.popular ? 'ring-4 ring-purple-500' : ''
                      } ${isFlipped ? 'absolute inset-0' : 'relative'}`}>
                      <div className="p-6 lg:p-8 flex flex-col h-full">
                        {pkg.badge && (
                          <div className={`inline-block px-4 py-2 bg-gradient-to-r ${pkg.color} text-white rounded-xl text-sm font-semibold mb-4 w-fit`}>
                            {pkg.badge}
                          </div>
                        )}
                        ...
                        <button className={`w-full py-4 bg-gradient-to-r ${pkg.color} text-white rounded-xl font-semibold hover:shadow-lg transition-all mt-auto`}>
                          ...
                          <div className="space-y-5 overflow-y-auto flex-1">
                            {(() => {
                              ...
                              return column?.items.map((item, idx) => (
                                <div key={idx} className="p-4 lg:p-6 bg-white/10 rounded-xl hover:bg-white/15 transition-all">
                                  <p className="font-semibold text-lg lg:text-xl mb-3">{item.label}</p>
                                  <p className="text-white/90 text-sm lg:text-base leading-relaxed">{item.value}</p>
                                </div>
                              ));
                            })()}
                          </div>
    ...
    <div className="py-14 bg-white pb-10">
      <div className="max-w-4xl mx-auto px-6">
        <div className="space-y-6">
          ...
          >
            <button
              onClick={() => window.open('https://forms.gle/7MLLDNpfCosnB56k7', '_blank')}
            ...
            >
              <div className="text-center">
                ...
                <div className="text-base font-normal opacity-90">
                  Натисність, щоб заповнити форму та дізнайтеся приблизну вартість та терміни впровадження саме для вашої компанії
                </div>
              </div>
            </button>
          </motion.div>
          ...
          <div className="text-center">
            <span className="text-2xl font-semibold text-gray-700">АБО ЗВ'ЯЖІТЬСЯ З НАМИ</span>
          </div>
          ...
          >
            <div className="flex items-center justify-center gap-4 flex-wrap">
              <a
                ...
                className="flex items-center justify-center w-16 h-16 bg-white rounded-full hover:shadow-lg hover:scale-110 transition-all"
                title="Telegram"
              >
                <img alt="Telegram" className="w-10 h-10" src="https://img.icons8.com/color/48/telegram-app.png" />
              </a>
              {/* Viber */}
              <a
                ...
                title="Viber"
              >
                <img alt="Viber" className="w-10 h-10" src="https://img.icons8.com/color/48/viber.png" />
              </a>
              <a
                href="tel:+380950571649"
                className="flex items-center gap-2 px-4 py-3 bg text-black rounded-full hover:shadow-lg hover:scale-110 transition-all"
              >
                <div className="w-9 h-9 bg-white rounded-full flex items-center justify-center border-3">
            ...
            {steps.map((step, index) => (
              ...
              >
                <div className="bg-white rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-2 flex flex-col w-full h-full">
                  ...
                  <h3 className="text-xl font-semibold text-gray-900 mb-2 text-center flex-shrink-0">{step.title}</h3>
                  <p className="text-gray-600 text-center mb-3 flex-grow">{step.description}</p>
                  <p className="text-purple-600 font-semibold text-sm text-center flex-shrink-0">⏱️ {step.duration}</p>
                </div>
              </motion.div>
            ))}
          </div>
    ...
    <div className="py-8 pb-3 bg-gradient-to-br from-purple-900 via-purple-800 to-pink-800 text-white">
      <div className="max-w-4xl mx-auto px-6 text-center">
        ...
        >
          ...
          <h3 className="text-2xl md:text-3xl font-semibold mb-8 text-purple-200">Контакти</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-5 bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/20 transition-all duration-300 hover:bg-white/15">
              <div className="flex items-center gap-3 sm:gap-4">
                <a href="tel:+380950571649" className="hover:scale-110 transition-transform" title="Телефон">
                  <img alt="Phone" className="w-7 h-7" src="https://img.icons8.com/ios-filled/50/ffffff/phone.png" />
                </a>
                <a href="https://t.me/bonnie_benay" target="_blank" rel="noreferrer" className="hover:scale-110 transition-transform" title="Telegram">
                  <img alt="Telegram" className="w-8 h-8" src="https://img.icons8.com/color/48/telegram-app.png" />
                </a>
                <a href="viber://chat?number=%2B380950571649" className="hover:scale-110 transition-transform" title="Viber">
                  <img alt="Viber" className="w-8 h-8" src="https://img.icons8.com/color/48/viber.png" />
                </a>
              </div>

              <div className="hidden sm:block w-px h-8 bg-white/20 mx-1"></div>

              <a href="tel:+380950571649" className="text-base sm:text-lg text-purple-100 hover:text-white transition-all duration-300 font-medium hover:scale-105">
                +380 95 057 16 49
              </a>
            </div>
          ...
          </div><div className="flex items-center max-w-3xl mx-auto my-3">
            <div className="flex-1 h-px bg-gradient-to-r from-transparent via-purple-400 to-purple-400"></div>
            <div className="flex-1 h-px bg-gradient-to-l from-transparent via-purple-400 to-purple-400"></div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
            {[
              { icon: <Zap className="w-8 h-8" />, title: "Швидко", desc: "На 80% швидша адаптація" },
              { icon: <CheckCircle className="w-8 h-8" />, title: "Впевнено", desc: "10.000+ опрацьованих документів" },
              { icon: <Users className="w-8 h-8" />, title: "З підтримкою", desc: "Не залишимо наодинці" }
            ].map((item, index) => (
              <div
                key={index}
...
const Footer = memo(() => {
  ...
  const faqData = useMemo(() => [
    {
      ...
      a: (<>Гнучкість та фокус на результаті. Ми не підлаштовуємо ваш бізнес під можливості програми, а навпаки — адаптуємо інструменти під ваші <b>звичаї</b>. Це створення живої архітектури, яка працює на вас, поки ви фокусуєтесь на стратегії.
      </>)
    },
    {
      q: "Що, якщо мій бізнес занадто специфічний і тримається на «особистій магії»?",
      a: (<>Саме тому проєкт називається <b>ЗВИЧАЇКА</b>. Ми не насаджуємо шаблонні процеси. Якщо ваш успіх базується на особистих зв'язках — ми опишемо систему підтримки цих зв'язків. Будь-яка «магія», що приносить гроші, має свій алгоритм. Ми знайдемо його і зафіксуємо, щоб результативність не залежала від настрою чи пам'яті окремих людей.
      </>)
    },
    {
    ...
    },
    {
      ...
      a: (
        <>Все залежить від масштабу:
          <p><b>Порядок на вчора:</b> 1-2 тижні.</p>
          <p><b>Корпоративна культура та ШІ:</b> 3–5 тижнів.</p>
          <p><b>Цифровий мозок компанії:</b> 2–3 місяці.</p>
        </>)
    },
    {
    ...
    },
    {
      ...
      a: (
        <>Зовсім ні. Хоча пакети сформовані для комплексного вирішення задач, <b>ЗВИЧАЇКА</b> — це передусім про доцільність. Ми можемо зайти в проєкт точково, щоб вирішити конкретний «біль»:
          <p><strong>Міграція:</strong> Наприклад, якщо ви хочете переїхати з однієї таск-менеджмент системи в іншу (з Trello в Asana чи Notion) без втрати даних та нервів команди.</p>
          <p><strong>Систематизація:</strong>Якщо потрібно просто перетворити хаос із файлів на Google Диску на логічну, структуровану базу даних, де кожен документ має своє місце.</p>
          <p><strong>Окремий процес:</strong>Описати лише один складний вузол (наприклад, найм або обробку замовлень).</p>
        </>)
    }
  ], []);
  ...
  return (
    <div className="bg-gradient-to-b from-gray-900 to-black text-white py-2">
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-center items-center gap-6">
          ...
          <p className="text-center">
            © 2026 Розробник drgnvlnc@gmail.com
          </p>
      ...
      {/* FAQ Modal */}
      {isFaqOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setIsFaqOpen(false)}>
          <div className="bg-white rounded-3xl max-w-3xl w-full max-h-[80vh] overflow-y-auto shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 bg-gradient-to-r from-purple-600 to-pink-600 text-white p-6 rounded-t-3xl flex items-center justify-between z-10">
              ...
              <button onClick={() => setIsFaqOpen(false)} className="hover:bg-white/20 rounded-2xl p-2 transition-all">
                <X size={28} />
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setIsPrivacyOpen(false)}>
          <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[85vh] overflow-y-auto shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 bg-gradient-to-r from-purple-600 to-pink-600 text-white p-6 rounded-t-3xl flex items-center justify-between z-10">
              ...
              <button onClick={() => setIsPrivacyOpen(false)} className="hover:bg-white/20 rounded-full p-2 transition-all">
                <X size={28} />
              </button>
            </div>
            ...
            <div className="p-8 space-y-6 text-gray-700 leading-relaxed">
              ...
              <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-6 rounded-2xl border-l-4 border-purple-600">
                <p className="text-lg text-gray-800">
                  Ми цінуємо вашу довіру та дбаємо про безпеку вашої інформації. Поки ви занурюєтесь у культуру своєї компанії та плануєте її розвиток за чашкою чаю, проект <strong>«ЗВИЧАЇКА»</strong> забезпечує надійну роботу систем, зокрема й захист ваших даних.
                </p>
              </div>
              ...
              <section>
                ...
                <p className="mb-3">
                  Ця Політика визначає порядок збору, обробки та зберігання персональних даних користувачів сайту. Ми розробили її відповідно до:
                </p>
                  ...
                  <li className="flex items-start gap-2">
                    ...
                    <span>
                      <a
                        ...
                        className="text-purple-600 hover:text-purple-800 underline font-medium"
                      >
                        Загального регламенту про захист даних (GDPR) Європейського Союзу
                      </a>
                    </span>
                  </li>
                </ul>
              ...
              <section>
                ...
                <p className="mb-4">
                  Ми збираємо дані лише у випадку, якщо ви вирішили співпрацювати з нами та добровільно надали інформацію через форми зворотного зв'язку або анкети.
                </p>
                <div className="space-y-3">
                  ...
                  <div className="bg-gray-50 p-4 rounded-xl">
                    ...
                    <p>Сайт автоматично фіксує технічну інформацію (IP-адреса, cookies, дані браузера). Ці дані зберігаються тимчасово та використовуються для коректної роботи сайту, аналітики відвідуваності та захисту від спаму.</p>
                  </div>
                </div>
              </section>
              ...
              <section>
                ...
                <ul className="space-y-2 pl-4">
                  <li className="flex items-start gap-3">
                    <span className="text-purple-600 mt-1">•</span>
                    <span>Надання консультацій та підготовка індивідуальних пропозицій</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <span className="text-purple-600 mt-1">•</span>
                    <span>Автоматизація процесів: після заповнення форми ваші дані передаються до нашої внутрішньої системи обліку для систематизації роботи</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <span className="text-purple-600 mt-1">•</span>
                    <span>Інформування про послуги, новини проекту та спеціальні пропозиції (у кожному листі ви знайдете кнопку «Відписатися»)</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <span className="text-purple-600 mt-1">•</span>
                    <span>Виконання договірних та переддоговірних зобов'язань</span>
                  </li>
                </ul>
              </section>
              ...
              <section>
                <h3 className="text-2xl font-semibold text-gray-900 mb-4 flex items-center gap-2">
                  <span className="text-purple-600">4.</span> Зберігання та захист інформації
                </h3>
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-purple-50 p-4 rounded-xl border-l-4 border-purple-400">
                    ...
                    <p>Після відправлення дані автоматично потрапляють до нашої внутрішньої системи обліку клієнтів, побудованої на базі захищених хмарних сервісів Google та CRM.</p>
                  </div>
                  <div className="bg-purple-50 p-4 rounded-xl border-l-4 border-purple-400">
                    <p className="font-semibold text-gray-900 mb-2">🛡️ Обмежений доступ:</p>
                    <p>Доступ до бази даних мають лише авторизовані спеціалісти проекту «ЗВИЧАЇКА» із застосуванням двофакторної автентифікації. Ми не зберігаємо персональні дані на сервері сайту довше, ніж це необхідно для їх технічної передачі в систему обліку.</p>
                  </div>
                </div>
              </section>
              ...
              <section>
                <h3 className="text-2xl font-semibold text-gray-900 mb-4 flex items-center gap-2">
                  <span className="text-purple-600">5.</span> Передача третім сторонам
                </h3>
                <p>
                  Ми <strong>не продаємо і не передаємо</strong> ваші дані третім особам без вашої згоди, за винятком випадків, передбачених законодавством України, або використання технічних сервісів (CRM, Google Workspace), які забезпечують процес зберігання та обробки даних на умовах конфіденційності.
                </p>
              </section>
              ...
              <section>
                ...
                <ul className="space-y-2 pl-4">
                  <li className="flex items-start gap-3">
                    <span className="text-purple-600 mt-1">✓</span>
                    <span>Знати, які дані ми обробляємо, та отримати до них доступ</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <span className="text-purple-600 mt-1">✓</span>
                    <span>Вимагати виправлення або повного видалення ваших даних із нашої системи</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <span className="text-purple-600 mt-1">✓</span>
              ...
              <section>
                ...
                <p>
                  Дані зберігаються протягом періоду, необхідного для досягнення цілей обробки, або до моменту відкликання вашої згоди.
                </p>
              </section>
              ...
              <section className="bg-gradient-to-r from-purple-600 to-pink-600 text-white p-6 rounded-2xl">
                ...
                <div className="space-y-2">
                  <p>
                    <strong>Email:</strong>{' '}
                    <a href="mailto:hanna.ws.g@gmail.com" className="underline hover:text-purple-200 transition-colors">
                      hanna.ws.g@gmail.com
                    </a>
                  </p>
                  <p>
                    <strong>Телефон:</strong>{' '}
                    <a href="tel:+380950571649" className="underline hover:text-purple-200 transition-colors">
                      +380 95 057 16 49
                    </a>
                  </p>
                </div>
              ...
              <section className="bg-gray-100 p-5 rounded-xl border-t-4 border-gray-300">
                <p className="text-sm text-gray-700">
                  <strong className="text-gray-900">Останнє оновлення:</strong> 01 лютого 2026 р.<br />
                  <span className="text-gray-600">
                    Ми залишаємо за собою право оновлювати цю політику. Зміни набувають чинності з моменту публікації на сайті.
                  </span>
                </p>
              </section>
            </div>
